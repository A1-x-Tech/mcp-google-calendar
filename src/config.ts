...
/** Default API host — Calendar API paths start with calendar/v3/. */
...
export const MISSING_CREDENTIALS_MESSAGE =
  ...
  "GOOGLE_CALENDAR_ACCESS_TOKEN with a short-lived access token. " +
...
export function hasCredentials(config: GoogleCalendarConfig): boolean {
 ...
 *
 *   GOOGLE_CALENDAR_CLIENT_ID      OAuth2 client id      (refresh flow, recommended)
 *   GOOGLE_CALENDAR_CLIENT_SECRET  OAuth2 client secret  (refresh flow)
 *   GOOGLE_CALENDAR_REFRESH_TOKEN  OAuth2 refresh token  (refresh flow)
 *   GOOGLE_CALENDAR_ACCESS_TOKEN   static access token (alternative; expires in ~1h)
 *   GOOGLE_CALENDAR_API_BASE       API root override (default https://www.googleapis.com)
 *   GOOGLE_CALENDAR_TIMEOUT_MS     per-request timeout (default 60000)
 *   GOOGLE_CALENDAR_MAX_RETRIES    retries on transient errors (default 3)
 */
export function loadConfig(): GoogleCalendarConfig {
  ...
  if (oauthProvided > 0 && oauthProvided < 3) {
    throw new ConfigError(
      "GOOGLE_CALENDAR_CLIENT_ID, GOOGLE_CALENDAR_CLIENT_SECRET and GOOGLE_CALENDAR_REFRESH_TOKEN must be set together (OAuth2 refresh flow).",
      "incomplete_oauth_config",
    );
  }

  const timeoutMs = Number(process.env.GOOGLE_CALENDAR_TIMEOUT_MS);
  const maxRetries = Number(process.env.GOOGLE_CALENDAR_MAX_RETRIES);
  ...
  return {
    ...
    refreshToken,
    accessToken,
    apiBase: process.env.GOOGLE_CALENDAR_API_BASE || DEFAULT_BASE,
