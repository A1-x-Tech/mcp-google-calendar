...
test("a partial OAuth triple reports incomplete_oauth_config", () => {
  assert.equal(reasonOf({ GOOGLE_CALENDAR_CLIENT_ID: "id" }), "incomplete_oauth_config");
  assert.equal(
    reasonOf({ GOOGLE_CALENDAR_CLIENT_ID: "id", GOOGLE_CALENDAR_CLIENT_SECRET: "secret" }),
  ...
  assert.equal(
    reasonOf({ GOOGLE_CALENDAR_REFRESH_TOKEN: "rt", GOOGLE_CALENDAR_ACCESS_TOKEN: "at" }),
    "incomplete_oauth_config",
  );
});
...
test("a static access token alone is enough", () => {
  withEnv({ GOOGLE_CALENDAR_ACCESS_TOKEN: "at" }, () => {
    const config = loadConfig();
  ...
  withEnv(
    {
      ...
      GOOGLE_CALENDAR_TIMEOUT_MS: "not-a-number",
...
test("numeric overrides are honored when valid", () => {
  withEnv(
    {
      GOOGLE_CALENDAR_ACCESS_TOKEN: "at",
      GOOGLE_CALENDAR_TIMEOUT_MS: "1000",
      GOOGLE_CALENDAR_MAX_RETRIES: "0",
    },
    () => {
      const config = loadConfig();
...
test("the API base override is honored", () => {
  withEnv({ GOOGLE_CALENDAR_ACCESS_TOKEN: "at", GOOGLE_CALENDAR_API_BASE: "https://example.test" }, () => {
