...
(`io.github.A1-x-Tech/mcp-google-calendar`). Check before publishing — all three
...
2. `npm publish` — runs `typecheck` + `test` + `build` via `prepublishOnly` / `prepare`.
   The package is scoped (`@a1-x-tech/mcp-google-calendar`) and carries
   `"publishConfig": { "access": "public" }` — without it a scoped publish defaults
   to private and fails on a free plan.
...
2. Point it at `https://github.com/A1-x-Tech/mcp-google-calendar`. LobeHub pulls the README,
   tool list and install config (`npx -y @a1-x-tech/mcp-google-calendar`) automatically.
